...
function isDebugEnv() {
    const env = window.appEnv || process.env.APP_ENV || process.env.NODE_ENV || '';
        ...
        methods: {
            ...
            startStatePolling() {
                ...
                try {
                    ...
                    if (conn) {
                        if (isDebugEnv()) conn.bind('connected', () => { console.log('[Poll] Echo connected'); });
                        if (isDebugEnv()) conn.bind('disconnected', () => { console.warn('[Poll] Echo disconnected'); });
                        if (isDebugEnv()) conn.bind('error', (err) => { console.error('[Poll] Echo error', err); });
                    }
                } catch (e) { if (isDebugEnv()) console.warn('[Poll] Echo bind failed', e); }
            ...
            addMessage(msg) {
                axios.post("/bid/messages", msg).then((res) => {
                    ...
                    if (err.response && err.response.data && err.response.data.message &&
                        err.response.data.message.includes('Harga bid ini sudah diambil user lain')) {
                        // Langsung fetch data terbaru dari server
                        axios.get(`/bid/state/${window.productSlug}`).then((res) => {
                            const data = res.data || {};
                            const highest = Number(data.highest);
                            const msgs = Array.isArray(data.messages) ? data.messages : [];
