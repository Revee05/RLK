                ...
                <tbody>
                    @forelse($merchProducts as $merchProduct)
                        <tr>
                            ...
                            {{-- Nama Produk --}}
                            <td><span class="shrinkable-name"
                                    title="{{ $merchProduct->name }}">{{ $merchProduct->name }}</span></td>
                            {{-- Kategori --}}
                            <td>
                                <span class="shrinkable-category"
                                    title="{{ $merchProduct->categories->pluck('name')->join(', ') }}">
                            ...
                            <td>
                                @php
                                    $maxDiscount = $merchProduct->variants
                                ...
                                @foreach ($merchProduct->variants as $variant)
                                    @if ($variant->images->first())
                                        <div class="d-inline-block text-center me-1">
                                            <img src="{{ asset($variant->images->first()->image_path) }}" alt="Image"
                                                width="40" class="mb-1">
                            ...
                            {{-- Action --}}
                            <td>
                                <a href="{{ route('master.merchProduct.edit', $merchProduct->id) }}"
                                    class="btn btn-warning btn-sm">Edit</a>
                                <form action="{{ route('master.merchProduct.destroy', $merchProduct->id) }}" method="POST"
        ...
        $(document).ready(function() {
            var table = $('#merchProductTable').DataTable({
                "columnDefs": [{
                        "orderable": false,
                        "targets": [2, 8, 9, 10]
                    } // Disable sort for Categories, Variants, Images, Action
                ]
            });
