                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group row mb-0">
                                <label class="col-sm-4 font-weight-bold">Telephone</label>
                                <div class="col-sm-8">
                                    : {{ $order->phone ?? $detailBid->winner->hp ?? '-' }}
                                </div>
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group row mb-0">
                                ...
                                <div class="col-sm-8">
                                    : Rp.
                                    {{ number_format( optional($detailBid->bid->first())->price ?? optional($order)->bid_terakhir ?? $detailBid->final_price ?? $detailBid->price, 0, ',', '.') }}
                                </div>
                            </div>
                            <div class="form-group row mb-0">
                            ...
                            <div class="form-group row mb-0">
                                ...
                                <div class="col-sm-8">
                                    : {!! optional($order)->status_txt ?? '<span
                                        class="badge bg-warning text-white rounded-1">Belum diCO oleh pemenang</span>'
                                    !!}
                                    @if($order)
                                    <div class="small text-muted mt-1">
                                        Metode:
                                        {{ optional($order)->payment_method ? ucfirst(optional($order)->payment_method) : '-' }}
                                        @if(optional(optional($order)->paid_at)->format)
                                        &nbsp;|&nbsp; Dibayar:
                                        {{ optional(optional($order)->paid_at)->format('d M Y H:i') }}
                                        @endif
                                    </div>
                                    @endif
                                </div>
